/*  Program: price.cpp
    ...
    Class: CSCI 140
    Date: 10/08/2018
    Description: Program reads in price and store item into 2D array. prints the prices in table form.

    I certify that the code below is my own work.

	Exception(s): This is modified book code, with function and certain implementations included.
*/
...
void printPrices(PriceType table, int numOfRows, int numOfCols) {
	...
	for (int row = 0; row < numOfRows; row++) {
		for (int col = 0; col < numOfCols; col++) {
			cout << setw(10) << fixed << setprecision(2) << table[row][col];
		}
		cout << endl;
	}
}
